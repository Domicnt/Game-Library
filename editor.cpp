...
bool Editor::editBody(Physics& physics, Graphics& graphics, Input& input, b2Body* body)
{
	...
	case line:
		...
		{
			physics.addFixtureToBody(body, { {point.x * graphics.camera.scaling, point.y * graphics.camera.scaling}, {position.x * graphics.camera.scaling, position.y * graphics.camera.scaling} });
	...
	case circle:
		...
		{
			physics.addCircularFixtureToBody(body, Functions::distToPoint(point, position) * graphics.camera.scaling, { point.x * graphics.camera.scaling, point.y * graphics.camera.scaling });
			defined = false;
		}
		break;
	case polygon:
